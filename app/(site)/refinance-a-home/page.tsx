...
import RefinanceReason from "@/components/refinance-a-home/Refinance-Reason";
import CurrentProgram from "@/components/refinance-a-home/Current-Program";
...
const BuyAHomePage = () => {
  ...
  const [data, setData] = useState<RefinanceProspect | HomeBuyer>({
    propertyType: "",
    intendedUse: "",
    refinanceReason: "",
    currentProgram: "",
  ...
  const SubmitUser = async (e: any) => {
    e.preventDefault();
    const response = await fetch("/api/refinance-a-home", {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      toast.success(
        "Thank you for your submission! We will be in contact with you via phone or email."
      );
    }
  };
  ...
  const PageDisplay = () => {
    if (page == 0) {
      ...
      return <RefinanceReason data={data} setData={setData} />;
    } else if (page == 3) {
      return <CurrentProgram data={data} setData={setData} />;
    } else if (page == 4) {
  ...
  return (
    <div className="bg-white w-full h-screen min-h-full flex flex-col pt-20 sm:px-6 lg:px-8 z-100 mf:h-screen">
